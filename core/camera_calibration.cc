...
Cameras CameraCalibration::calibrate(int nx,int ny, float square_size ) {
  ...
  cv::Matx33d E_;
  cv::Matx33d F_;
  vector<double> D1(5,0);
  vector<double> D2(5,0);
  vector<cv::Point2f> temp1(n); //harvest points of corners for one image
  vector<cv::Point2f> temp2(n);
  vector<cv::Point3f> objectPoints;
  vector<vector<cv::Point2f>> VPoints1(0);
  vector<vector<cv::Point2f>> VPoints2(0);
  ...
  for (int i = 0; i < len; i++) {
    ...
    result1 = cv::findChessboardCorners(img1, cv::Size(nx,ny), temp1); // Finds the positions of
    result2 = cv::findChessboardCorners(img2, cv::Size(nx,ny), temp2); // internal corners of the chessboard
    if (result1 && result2) {
      cv::cornerSubPix(img1, temp1, cv::Size(5, 5), cv::Size(-1,-1), // Refines the corner locations
          cv::TermCriteria(cv::TermCriteria::EPS + cv::TermCriteria::COUNT, 30, 0.01));
      cv::cornerSubPix( img2, temp2, cv::Size(11, 11), cv::Size(-1,-1),
          cv::TermCriteria( cv::TermCriteria::EPS + cv::TermCriteria::COUNT, 30, 0.01));
  ...
  vector<vector<cv::Point3f> > VObjectPoints( count_of_pairs);
  ...
  std::cout << "size VObjPoint: "<<VObjectPoints.size() << "\n";
  std::cout <<"size VPoint1: " <<VPoints1.size() << " Vpoint2: " << VPoints2.size() << "\n";
  cv::setIdentity( K1_);
  cv::setIdentity( K2_);
...
void CameraCalibration::addImagePair(const cv::Mat &image1, const cv::Mat &image2) {
