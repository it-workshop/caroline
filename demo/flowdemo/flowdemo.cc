...
bool FlowDemo::Init() {
  ...
  optical_frow_processor_ = core::OpticalFlowProcessor::Create(config_);

  return optical_frow_processor_ && cap_.isOpened();
}
...
int FlowDemo::Run() {
  ...
  for ( ; ; ) {
    ...
    if (prevgray_.data) {
      flow_ = optical_frow_processor_->Process(prevgray_, gray_);
